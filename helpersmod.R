...
sensitivityAnalysis<-function(indicator,foldername,resultsShock,params,export=FALSE,plotrgl=FALSE){
	...
	min<-min(resultsShock[which(!is.na(shocks$ExitP)),params[1]])
	max<-max(resultsShock[which(!is.na(shocks$ExitP)),params[1]])
	...
	z.pred<-matrix(pred.m$mean,nrow=n.grid,ncol=n.grid)
	
	scatter3D(z = dataOriginal$z, x = dataOriginal$x, y = dataOriginal$y, pch = 18, cex = 2, 
						theta = -40, phi = 30, ticktype = "detailed", 
						xlab = expression(theta), ylab = expression(phi), zlab = "",
						surf = list(x = x.grid, y = y.grid, z = z.pred, facets=NA,fit=fitpoints$mean),
						main = "",colkey=FALSE)
