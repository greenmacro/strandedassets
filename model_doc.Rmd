...
AG: This is the number of period after which the innovative sectors enters the financial market. The idea is that at first, the innovative sector is too small to attract financial flows but once it has grown enough, it tries to rise funds via an IPO. In the original paper, we've played with that parameter and shown that if the innovators try to enter the financial market too soon it hurts them because they can't raise enough money via the IPO and thus are over indebted. The $M_i$ is the stock of cash that innovators hold after the IPO and which is used to reduce the quantity of loans or pay dividends.
...
AG: I think we're not worried about the case where consumption is above income but rather the case when consumption is negative because of negative expected income (this happens when the non-performin loans of the conventional sector is accounted for in the dividends from banks). We could change that set of equation to distinguish between income and capital loss.
...
AG: It is the case right now, but the equations could be simplified.
...
AG: There are no pensions in the system. Regarding money holding for capitalists: we assume there's a demand for money steming from consumption purposes, a demand of money steming from the portfolio equation and finaly the money held in the end depends on capital gains, expectations, etc. because money is the buffer stock.
