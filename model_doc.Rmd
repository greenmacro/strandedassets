...
Capital productivity of capital type $x$ in sector $y$ is denoted $pr_{yx}$, so labor productivity is the product $pr_{yx}\ell_x$. Average labor productivity in sector $x$, $\overline{pr}^N_x$ (denoted `Aprx` in the code) is then given by real output from the sector, $y_x$, divided by sector employment,
$$\overline{pr}^N_x = \frac{y_x}{N_x}$$
...
$$\omega^T_x=\Omega_0 + \Omega_1 \log \overline{pr}^N_x + \Omega_2 \log \frac{N_\text{tot}}{LF}$$
...
```
[_Note:_ I would think that it would be proportional to the average productivity, more or less, with fluctuations due to the (un)employment rate. E.g., `omegaTx = Aprx * (Omega0 + Omega2 * log(Ntot/LF))`]
...
cc1=(cc0+abs(cc0))/2
```
[_Note:_ This could be modified in future to set a minimum level of consumption equal to some fraction of previous period consumption, for example by setting `cc1=(cc0+fcmin*cc1(-1) + abs(cc0-fcmin*cc1(-1)))/2`. This captures the case in which some capitalists go thoroughly bankrupt and enter the workforce while others go temporarily bankrupt but keep up expenditure and manage to get new loans and yet others continue to consume.]
...
### Wealth
As noted above, consumption is paid for out of both current income and wealth. For either capitalists or workers (denoted generically by subscript $i$), nominal wealth is given by previous period wealth plus disposable income net of consumption,
$$V_i = V_{i,-1} + \left(YD_i - C_i\right)$$
Purchasing power out of wealth is then nominal wealth deflated by the cost of consumption goods,
$$v_i = \frac{V_i}{p_c}$$
```{r eval=FALSE}
Vc=Vc(-1) + (YDc-Cc)
vc=Vc/pc
...
vw=Vw/pc
```
Capitalists also make investment decisions out of their expected wealth in the next period, which they calculate using the same formula as above, but with expected disposal income,
$$V_c^e = V_{i,-1} + \left(YD_i^e - C_i\right)$$
```{r eval=FALSE}
Vce=Vc(-1) + YDce - Cc
```
We further distinguish capitalists' financial wealth $V_{ec}$, equal to the value of equity shares plus money held for financial reasons (as opposed to transactions). Denoting the number of shares in sector $s$ by $e_s$, the equity price by $p_{s,e}$, and money that capitalists wish to hold for financial reasons by $M_c^{D,f}$,
...
[_Note:_This could be modified in future, because CG can, in principle, differ from the cum of CGc, CGk and CGi.]
...
## Money and banking
Total money in the system is $M_s$, the sum of money held by wage-earners, $M_w$ and capitalists, $M_c$.  Also, innovators hold a stock $M_i$ of cash after their IPO, which is used to reduce the quantity of loans or pay dividends.
```{r eval=FALSE}
Ms=Mw+Mc+Mi
```
...
After innovative firms go public, they have a store of cash. Initially they have no cash on hand, and they operate with loans. When they first go public, the value of money holdings in the previous period is then zero, $M_{i,-1} = 0$. When they first go public, they issue 100 shares of new equity, $e_i^s = 100$ at the market's expected price, $p_i^e$. They add to cash holdings from their profits, $F_i$, net of new investment $I_i$ and dividends $FD_i$. The innovative firms put a priority on growth and paying dividends to investors, so from the time they go public onwards, the they use their money for those purposes first,
...
### Loans
Loans to the innovative sector are discussed above. For the consumer and conventional capital goods sectors, the basic formula is that loans equal previous period loans, plus investment net of profits (or zero, if profits exceed investment), less the value of new equity issues. For an arbitrary sector $i\in\{c,k\}$,
$$L_i = L_{i,-1} + \max\left(I_i-F_i,0\right) - e_i^s * p_i^e$$
Note that the net effect may be either an addition of new loans or a the paying down of prior loans. Also, for the conventional capital goods sector, if output or profits are zero because the innovative sector has superseded it, then there is no demand for loans. These are captured in the following equations:
```{r eval=FALSE}
Lc=Lc(-1)+Ifc-esc*pce
Lk=ifelse(yk!=0&Fk>=0,Lk(-1)+Ifk-esk*pke,0)
```
[_Note:_ This is where to alter the behavior towards dividends. Firms should strive to meet dividend payments.]

The total demand for loans is the sum of loans to each productive sector,
$$L_d = sum_{i\in\{c,k,i\}} L_i$$
```{r eval=FALSE}
Ld=Lc+Lk+Li
```
